...
## 2. 获取交易对深度数据   
  ...
  -----  | ---------
  mk_type | 交易区，比如 cnc
  coinname | 币名，比如: coinname=btc, 获取btc/cnc交易对的深度数据
...
## 3. 获取交易对历史成交数据   
  ...
  -----  | ---------
  mk_type | 交易区，比如 cnc
  coinname | 币名，比如: coinname =btc, 获取btc/cnc交易对的成交数据
...
## 7. 我的账户余额   
  ...
  -----  | ---------| ---------
  key  | 是 | 公钥
  time | 是 | 发起请求时的Unix时间戳，单位秒，不是毫秒
  md5  | 是 | 鉴权md5，md5=md5("{key}\_{user_id}\_{skey}\_{time}"), user_id是用户登录后的数字ID，不是邮箱账号
 coins | 是 | 币种参数，可传入单币种或多币种。一次最多20个，多币种用英文逗号分隔。示例：btc或btc,cnc 
